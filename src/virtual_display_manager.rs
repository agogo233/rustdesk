...
pub fn get_cur_device_string() -> &'static str {
    match IDD_IMPL {
        IDD_IMPL_MYDESK => MYDESK_IDD_DEVICE_STRING,
...
pub fn plug_in_headless() -> ResultType<()> {
    match IDD_IMPL {
        IDD_IMPL_MYDESK => mydesk_idd::plug_in_headless(),
...
pub fn get_platform_additions() -> serde_json::Map<String, serde_json::Value> {
    ...
    match IDD_IMPL {
        IDD_IMPL_MYDESK => {
            let virtual_displays = mydesk_idd::get_virtual_displays();
            if !virtual_displays.is_empty() {
                map.insert(
                    "mydesk_virtual_displays".into(),
...
pub fn plug_in_monitor(idx: u32, modes: Vec<virtual_display::MonitorMode>) -> ResultType<()> {
    match IDD_IMPL {
        IDD_IMPL_MYDESK => mydesk_idd::plug_in_index_modes(idx, modes),
    ...
    match IDD_IMPL {
        IDD_IMPL_MYDESK => {
            let indices = if index == IDD_PLUG_OUT_ALL_INDEX {
                mydesk_idd::get_virtual_displays()
            } else {
                vec![index as _]
            };
            mydesk_idd::plug_out_peer_request(&indices)
        }
...
pub fn plug_in_peer_request(modes: Vec<Vec<virtual_display::MonitorMode>>) -> ResultType<Vec<u32>> {
    match IDD_IMPL {
        IDD_IMPL_MYDESK => mydesk_idd::plug_in_peer_request(modes),
        IDD_IMPL_AMYUNI => {
...
pub fn plug_out_monitor_indices(
    ...
    match IDD_IMPL {
        IDD_IMPL_MYDESK => mydesk_idd::plug_out_peer_request(indices),
        IDD_IMPL_AMYUNI => {
...
pub fn reset_all() -> ResultType<()> {
    match IDD_IMPL {
        IDD_IMPL_MYDESK => mydesk_idd::reset_all(),
...
pub mod amyuni_idd {
    ...
    // 2. MyDesk may crash and restart, but the virtual displays are kept.
    //
    ...
    //             if there're no virtual displays managed by MyDesk.
    pub fn plug_out_monitor(index: i32, force_all: bool, force_one: bool) -> ResultType<()> {
        ...
        //
        // 1. MyDesk plug in 2 virtual displays. (MyDesk)
        // 2. Other process plug out all virtual displays. (User manually)
        // 3. Other process plug in 1 virtual display. (User manually)
        // 4. MyDesk plug out all virtual displays in this call. (MyDesk disconnect)
        //
        // This is not a normal scenario, MyDesk will plug out virtual display unexpectedly.

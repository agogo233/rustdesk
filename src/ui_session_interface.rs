...
impl<T: InvokeUiSession> Session<T> {
    ...
    pub fn printer_response(&self, id: i32, path: String, printer_name: String) {
        ...
        let to = std::env::temp_dir().join(format!("mydesk_printer_{id}"));
    ...
    if handler.is_port_forward() {
        if handler.is_rdp() {
            ...
            if handler.args.len() != 3
                ...
                || port <= 0
            {
                handler.on_error("Invalid arguments, usage:<br><br> mydesk --port-forward remote-id listen-port remote-host remote-port");
            }

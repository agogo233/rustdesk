...
pub static ref T: std::collections::HashMap<&'static str, &'static str> =
    [
        ...
        ("Ready", "준비 완료"),
        ("Established", "연결됨"),
        ("connecting_status", "MyDesk 네트워크에 연결 중..."),
        ...
        ("OS Password", "OS 비밀번호"),
        ("install_tip", "UAC로 인해 경우에 따라 MyDesk가 원격 쪽에서 제대로 작동하지 않을 수 있습니다. UAC를 피하려면 아래 버튼을 클릭하여 시스템에 MyDesk를 설치하세요."),
        ("Click to upgrade", "업그레이드"),
        ("Configure", "구성"),
        ("config_acc", "데스크탑을 원격으로 제어하려면 MyDesk에 \"접근성\" 권한을 부여해야 합니다."),
        ("config_screen", "데스크탑에 원격으로 액세스하려면 MyDesk에 \"화면 녹화\" 권한을 부여해야 합니다."),
        ("Installing ...", "설치 중..."),
        ("Install", "설치하기"),
        ("Installation", "설치"),
        ...
        ("android_input_permission_tip1", "원격 장치에서 마우스나 터치로 Android 장치를 제어하려면 MyDesk가 \"접근성\" 서비스를 사용하도록 허용해야 합니다."),
        ("android_input_permission_tip2", "다음 시스템 설정 페이지로 이동하여 [설치된 서비스]를 찾아 들어가서 [MyDesk 입력] 서비스를 켜세요."),
        ...
        ("Turned off", "꺼짐"),
        ("Language", "언어"),
        ("Keep RustDesk background service", "MyDesk 백그라운드 서비스 유지"),
        ("Ignore Battery Optimizations", "배터리 최적화 무시"),
        ("android_open_battery_optimizations_tip", "이 기능을 비활성화하려면 다음 MyDesk 응용 프로그램 설정 페이지로 이동하여 [배터리]를 찾아서 입력하고 [제한 없음]을 선택 취소하세요"),
        ...
        ("config_input", "키보드로 원격 데스크탑을 제어하려면 MyDesk에 \"입력 모니터링\" 권한을 부여해야 합니다."),
        ("config_microphone", "원격으로 통화하려면 MyDesk에 \"오디오 녹음\" 권한을 부여해야 합니다."),
        ...
        ("still_click_uac_tip", "여전히 원격 사용자가 MyDesk를 실행하는 UAC 창에서 확인을 클릭해야 합니다."),
        ...
        ("View Mode", "보기 모드"),
        ("verify_mydesk_password_tip", "MyDesk 비밀번호 확인"),
        ...
        ("scam_text1", "알지 못하고 신뢰할 수 없는 사람이 전화를 걸어 MyDesk를 사용하고 서비스를 시작하라고 요청하는 경우 계속 진행하지 말고 즉시 전화를 끊으세요."),
        ...
        ("upgrade_mydesk_server_pro_to_{}_tip", "MyDesk Server Pro를 {} 버전 이상으로 업그레이드하세요!"),
        ...
        ("selinux_tip", "SELinux가 장치에서 활성화되어 있어 MyDesk가 제어된 상태로 제대로 작동하지 않을 수 있습니다."),
        ("Change view", "보기 변경"),
        ("Big tiles", "큰 타일"),
        ("Small tiles", "작은 타일"),
        ("List", "목록"),
        ...
        ("allow-only-conn-window-open-tip", "MyDesk 창이 열려 있을 때만 연결 허용"),
        ...
        ("floating_window_tip", "MyDesk 백그라운드 서비스를 유지하는 데 도움이 됩니다"),
        ...
        ("upgrade_remote_mydesk_client_to_{}_tip", "MyDesk 클라이언트를 원격 버전 {} 이상으로 업그레이드해 주세요!"),
        ...
        ("allow-insecure-tls-fallback-tip", "기본적으로 MyDesk는 TLS를 사용하여 프로토콜에 대한 서버 인증서를 검증합니다.\n이 옵션을 활성화하면 MyDesk는 인증 단계를 건너뛰고 인증 실패 시 진행합니다."),
        ("Disable UDP", "UDP 사용 안 함"),
        ("disable-udp-tip", "TCP만 사용할지 여부를 제어합니다.\n이 옵션을 활성화하면 MyDesk는 더 이상 UDP 2116을 사용하지 않고 대신 TCP 2116을 사용합니다."),
        ("server-oss-not-support-tip", "참고: MyDesk 서버 OSS에는 이 기능이 포함되어 있지 않습니다."),
        ...
        ("RustDesk cannot reach the desktop session on the remote device, check that a desktop session is running and that RustDesk can use it", "MyDesk가 원격 장치의 데스크톱 세션에 접근할 수 없습니다. 세션이 실행 중이고 MyDesk가 사용할 수 있는지 확인하세요"),
        ...
        ("RustDesk could not obtain a usable screen from the XDG Desktop Portal, the PipeWire library may be too old", "MyDesk가 XDG Desktop Portal에서 사용 가능한 화면을 가져오지 못했습니다. PipeWire 라이브러리가 너무 오래되었을 수 있습니다"),
        ("RustDesk could not load a GStreamer component needed for screen capture ({})", "MyDesk가 화면 캡처에 필요한 GStreamer 구성 요소를 불러오지 못했습니다 ({})"),

...
const MSG_TO_MYDESK_SIGNATURE_VERIFICATION: &str = "signature_verification";
...
pub(super) extern "C" fn cb_msg(
    ...
    match &target as _ {
        ...
        MSG_TO_MYDESK_TARGET => handle_msg_to_mydesk(id, content, len),
        _ => PluginReturn::new(
...
fn handle_msg_to_mydesk(id: String, content: *const c_void, len: usize) -> PluginReturn {
    let s = early_return_value!(
        std::str::from_utf8(unsafe { std::slice::from_raw_parts(content as _, len) }),
        ERR_CALLBACK_INVALID_MSG,
        "parse msg string"
    ...
    match &msg_to_mydesk.r#type as &str {
        MSG_TO_MYDESK_SIGNATURE_VERIFICATION => request_plugin_sign(id, msg_to_mydesk),
        t => PluginReturn::new(
            errno::ERR_CALLBACK_TARGET_TYPE,
            &format!(
                "Unknown target type '{}' for target {}",
                t, MSG_TO_MYDESK_TARGET
            ),
        ),
    }
}

fn request_plugin_sign(id: String, msg_to_mydesk: MsgToMyDesk) -> PluginReturn {
    let signature_data = early_return_value!(
        std::str::from_utf8(&msg_to_mydesk.data),
        ERR_CALLBACK_INVALID_MSG,

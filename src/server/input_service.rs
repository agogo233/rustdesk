...
impl VirtualInputState {
    fn new() -> Option<Self> {
        VirtualInput::new(
            ...
            // 1. Press "Command" key in MyDesk
            // 2. Move the physical mouse
            // 3. Press "V" key in MyDesk
            ...
            // we consider only MyDesk operates the controlled side.
            //
...
fn key_sleep() {
    ...
    // Though `/Applications/MyDesk.app/Contents/MacOS/mydesk --server` in terminal is ok.
    let now = Instant::now();
...
pub async fn lock_screen() {
    ...
    if #[cfg(target_os = "linux")] {
        ...
        // loginctl lock-session also not work, they both work run mydesk from cmd
        std::thread::spawn(|| {

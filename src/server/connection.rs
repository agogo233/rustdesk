...
pub struct Connection {
    ...
    file_timer: crate::MyDeskInterval,
...
impl Connection {
    pub async fn start(
        ...
        let mut conn = Self {
            ...
            hash,
            read_jobs: Vec::new(),
            timer: crate::mydesk_interval(time::interval(SEC30)),
            file_timer: crate::mydesk_interval(time::interval(SEC30)),
            file_transfer: None,
            view_camera: false,
            terminal: false,
        ...
        let mut second_timer = crate::mydesk_interval(time::interval(Duration::from_secs(1)));
            ...
            tokio::select! {
                ...
                _ = conn.file_timer.tick() => {
                    if !conn.read_jobs.is_empty() {
                        ...
                        conn.file_timer = crate::mydesk_interval(time::interval_at(Instant::now() + SEC30, SEC30));
                    }
                }
                    ...
                    if handle_fa {
                        ...
                        match fa.union {
                            ...
                            Some(file_action::Union::Receive(r)) => {
                                ...
                                // whenever got send/recv request, check peer version to ensure old version of mydesk
        ...
        if self.keyboard {
            if let Ok(displays) = display_service::try_get_displays() {
                ...
                if let Some(display) = displays.get(display_idx) {
                    ...
                    #[cfg(windows)]
                    if let Some(_ok) =
                        virtual_display_manager::mydesk_idd::change_resolution_if_is_virtual_display(
                            &name,
                            r.width as _,
                            r.height as _,
                        )
    ...
    async fn process_new_read_job(&mut self, mut job: fs::TransferJob, path: String) {
        ...
        self.file_timer = crate::mydesk_interval(time::interval(MILLI1));
    ...
    async fn send_printer_request(&mut self, data: Vec<u8>) {
        ...
        let path = format!("MyDesk://FsJob//Printer/{}", get_time());

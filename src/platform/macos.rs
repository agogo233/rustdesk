...
// remove just one app from all the permissions: tccutil reset All com.mydesk.mydesk
...
fn correct_app_name(s: &str) -> String {
    ...
    if let Some(bundleid) = get_bundle_id() {
        s = s.replace("com.mydesk.mydesk", &bundleid);
    }
...
pub fn start_os_service() {
    ...
    // Silent auto-update — disabled per security policy

...
g_arpsystemcomponent = {
    ...
    "Contact": {
        "msi": "ARPCONTACT",
        "v": "https://github.com/mydesk/mydesk",
    },
    "HelpLink": {
        "msi": "ARPHELPLINK",
        "v": "https://github.com/mydesk/mydesk/issues/",
    },
    "ReadMe": {
        "msi": "ARPREADME",
        "v": "https://github.com/mydesk/mydesk",
...
def make_parser():
    ...
    parser.add_argument(
        "--app-name", type=str, default="MyDesk", help="The app name."
    )
    parser.add_argument(
...
# The app executable is handled separately: it has its own component in MyDesk.wxs.
#
...
def put_app_exe_on_media2():
    ...
    target = Path(sys.argv[0]).parent.joinpath("Package/Components/MyDesk.wxs")
...
def replace_app_name_in_langs(app_name):
    ...
    for file_path in langs_dir.glob("*.wxl"):
        with open(file_path, "r", encoding="utf-8") as f:
            lines = f.readlines()
        for i, line in enumerate(lines):
            lines[i] = line.replace("MyDesk", app_name)

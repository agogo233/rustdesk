...
#define MYDESK_LIB_PATH "libmydesk.so"
typedef bool (*MyDeskCoreMain)();
bool gIsConnectionManager = false;

void print_help_install_pkg(const char* so);

// The bundle keeps the core library at lib/libmydesk.so next to the
...
static void* dlopen_bundled_lib() {
  ...
  *last_slash = '\0';
  char lib_path[PATH_MAX + sizeof("/lib/" MYDESK_LIB_PATH)];
  snprintf(lib_path, sizeof(lib_path), "%s/lib/%s", exe_path, MYDESK_LIB_PATH);
  if (access(lib_path, F_OK) != 0) return nullptr;
  void* libmydesk = dlopen(lib_path, RTLD_LAZY);
  if (!libmydesk) {
    char* error = dlerror();
    if (error != nullptr) {
...
bool flutter_mydesk_core_main() {
   void* libmydesk = dlopen_bundled_lib();
   if (!libmydesk) {
      libmydesk = dlopen(MYDESK_LIB_PATH, RTLD_LAZY);
   }
   if (!libmydesk) {
      fprintf(stderr,"Failed to load \"libmydesk.so\"\n");
      char* error;
   ...
   auto core_main = (MyDeskCoreMain) dlsym(libmydesk,"mydesk_core_main");
   char* error;
   if ((error = dlerror()) != nullptr) {
       fprintf(stderr, "Program entry \"mydesk_core_main\" is not found: %s\n", error);
       return false;
   }
   return core_main();
}

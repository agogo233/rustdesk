...
class MyDeskTerminal extends Terminal {
  ...
  void _handlePrivateOsc(String code, List<String> args) {
    ...
    if (args.length != 2 || !_osc52Selection.hasMatch(args.first)) {
      debugPrint('[MyDeskTerminal] Rejected malformed OSC 52 command');
      return;
    }
    if (args.last == '?') {
      debugPrint('[MyDeskTerminal] Rejected OSC 52 clipboard query');
      return;
    }
    final permission = _clipboardWritePermission();
    if (permission == TerminalClipboardWritePermission.denied) {
      debugPrint('[MyDeskTerminal] Rejected unauthorized OSC 52 write');
      return;
    }
    final selection = args.first;
    if (selection.isNotEmpty &&
        !selection.contains(_systemClipboardSelection)) {
      debugPrint('[MyDeskTerminal] Ignored unsupported OSC 52 selection');
      return;
    }
    if (selection.replaceAll(_systemClipboardSelection, '').isNotEmpty) {
      debugPrint('[MyDeskTerminal] Ignored unsupported OSC 52 selections');
    }
    final text = _decodeClipboardPayload(args.last);
    if (text == null) return;
    if (permission == TerminalClipboardWritePermission.unconfigured) {
      debugPrint('[MyDeskTerminal] Blocked OSC 52 write pending consent');
  ...
  Future<void> _writeClipboard(String text) async {
    ...
    debugPrint(
        '[MyDeskTerminal] OSC 52 clipboard write requires interaction');
    _onClipboardWriteBlocked?.call(text);
  }

  String? _decodeClipboardPayload(String payload) {
    if (payload.length > _maxBase64EncodedLength(_maxClipboardWriteBytes)) {
      debugPrint('[MyDeskTerminal] Rejected oversized OSC 52 payload');
      return null;
    }
    try {
      ...
      debugPrint('[MyDeskTerminal] Rejected malformed OSC 52 payload');
      return null;
    }
  }
